...
##
## ── 표의 수는 어디서 왔나 ──────────────────────────────────────────
## `최대`·`최소`·`폭` 은 **`tools/loop/jitter.sh` 가 잰 것**이다 (2026-09-15 ·
## 같은 기계 · **9판을 두 벌 = 18판**). 게이트가 견주는 수를 `Tol.obs` 로 흘려 모은다.
## **두 벌의 「더 나쁜 쪽」을 적는다** — band·cap 은 최대와 폭이 큰 쪽, floor 는 최소가
## 작고 폭이 큰 쪽. 한 벌만 보면 폭을 **작게** 본다: 둘째 벌에서 `COLLIDE.세로속력` 이
## 2.830 → **4.061**(폭 1.052 → 2.061), `USE.벌어짐` 의 폭이 0.536 → **1.595** 로
## 넓어졌고, `MOVE` 는 거꾸로 2.326 → 1.639 로 좁아졌다. **폭은 재는 벌마다 다르다.**
## **다시 재려면 `tools/loop/jitter.sh 9` 를 돌리고 이 표를 고쳐라** (표는 `NUMBERS` 17절).
## 표를 안 고치고 허용치만 만지면 여기가 빨개진다 — 그게 이 검사가 하는 일 전부다.
##
...
const ROWS := [
	...
	{"자리": "COLLIDE.세로속력", "파일": "res://tools/tests/measure_collide.gd", "상수": "TOL_RATE",
	 "꼴": "band", "잰값": 4.061, "폭": 2.061, "고장": 169.71,
	...
	{"자리": "USE.벌어짐", "파일": "res://tools/tests/measure_window.gd", "상수": "USE_MIN_SPAN",
	 "꼴": "floor", "잰값": 19.980, "폭": 1.595, "고장": 0.5,
