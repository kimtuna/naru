...
## (바퀴 3 속도 · 4 화면 · 5 방향 · 6 월드 · 7 충돌과 **같은 모양의 구멍**이다. NUMBERS 5절)
##
## **안 움직이면 「늘 한가운데」는 공짜다** — 그래서 걸은 거리도 같이 잰다.
## 카메라가 아예 없어도 플레이어가 화면 밖으로 나가는 데는 시간이 걸리므로,
## 판정은 「화면 안에 있나」가 아니라 **「중심에서 몇 px 벗어났나」**다.
##
## 헤드리스로 된다 — 캔버스 변환은 창 없이도 돈다. 창·배율은 measure_view.gd 가 잰다.
##
## 이름이 test_ 로 시작하지 않는다 — run_tests.gd 는 이 파일을 안 집는다.

const LOGICAL := Vector2(960.0, 540.0)     # NUMBERS 1절
const TILES := Vector2(20.0, 11.25)        # 960/48 · 540/48 — 줌이 1 일 때만 이 값이다
const SEC := 0.5                           # 한 방향으로 걷는 시간. 240px/s → 120px = 2.5칸
