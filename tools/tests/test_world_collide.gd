...
##
## 숫자는 전부 손으로 계산한 값이다: 타일 48 · 몸 반폭 22 · 틈 0.01.
## 예) 3번 칸(144px)에 오른쪽으로 부딪히면 몸 중심은 144 - 22 - 0.01 = **121.99**.
...
func test_open_world_moves_the_whole_motion() -> void:
	...
	var p := WorldCollide.move(Vector2(72, 72), Vector2(4.0, -3.0), _open())
	at(p, 76.0, 69.0, "빈 월드 이동")
...
func test_zero_motion_stays() -> void:
	var p := WorldCollide.move(Vector2(121.99, 72), Vector2.ZERO, _map(COAST))
	at(p, 121.99, 72.0, "안 움직이면 제자리")

## ── 바다에 못 들어간다 ────────────────────────────────────────────────

func test_stops_in_front_of_the_wall() -> void:
	var p := WorldCollide.move(Vector2(72, 72), Vector2(200, 0), _map(COAST))
	at(p, 121.99, 72.0, "해안에 정면으로 부딪힘")

func test_four_directions_stop_symmetrically() -> void:
	# 방 한가운데(120,120)에서 네 방향으로 500px 씩 — 어느 쪽이든 벽 앞 22.01px 에 선다.
	var m := _map(BOX)
	var want := {
		"오른쪽": [Vector2(500, 0), Vector2(169.99, 120)],
		"왼쪽": [Vector2(-500, 0), Vector2(70.01, 120)],
		"아래": [Vector2(0, 500), Vector2(120, 169.99)],
		"위": [Vector2(0, -500), Vector2(120, 70.01)],
	}
	for name in want:
		var p := WorldCollide.move(Vector2(120, 120), want[name][0], m)
		at(p, want[name][1].x, want[name][1].y, "%s 벽" % name)

func test_one_tick_does_not_tunnel() -> void:
	# 한 틱에 100칸을 가도 벽을 뚫으면 안 된다 — 지나가는 칸을 전부 본다.
	var p := WorldCollide.move(Vector2(72, 72), Vector2(5000, 0), _map(COAST))
	at(p, 121.99, 72.0, "한 틱 5000px")

func test_resting_on_the_wall_does_not_drift() -> void:
	# 벽에 붙은 자리에서 계속 밀면 값이 흔들리면 안 된다 (몸이 떨린다).
	var p := Vector2(121.99, 72)
	for i in 10:
		p = WorldCollide.move(p, Vector2(4, 0), _map(COAST))
	at(p, 121.99, 72.0, "벽에 10틱 밀기")

## ── 미끄러진다 ────────────────────────────────────────────────────────

func test_diagonal_into_the_coast_slides() -> void:
	# **이 바퀴의 핵심 주장.** 막힌 축만 죽고 나머지 축은 통째로 살아 있다.
	var p := WorldCollide.move(Vector2(72, 72), Vector2(200, 200), _map(COAST))
	at(p, 121.99, 272.0, "대각으로 해안에 붙음")
...
func test_slide_keeps_the_full_axis_speed() -> void:
	...
	var p := WorldCollide.move(Vector2(72, 72), step, _map(COAST))
	at(p, 121.99, 72.0 + 169.7056, "해안을 타고 내려간 거리")

func test_concave_corner_blocks_both_axes() -> void:
	var m := _map(["...#", "...#", "####"])
	var p := WorldCollide.move(Vector2(120, 72), Vector2(200, 200), m)
	at(p, 121.99, 73.99, "오목한 구석")

## ── 끼지 않는다 ───────────────────────────────────────────────────────

func test_body_is_narrower_than_a_tile() -> void:
	# 같으면 48px 통로에서 부동소수 한 톨에 걸려 낀다. 이 부등식이 아래 검사의 근거다.
...
func test_one_tile_corridor_is_passable() -> void:
	var m := _map(["#.#", "#.#", "#.#", "#.#"])
	var p := WorldCollide.move(Vector2(72, 24), Vector2(0, 500), m)
	at(p, 72.0, 524.0, "폭 1칸 통로 통과")

func test_leaves_a_wall_it_already_overlaps() -> void:
	# 몸이 이미 막는 칸에 걸쳐 있어도 **나오는 쪽은 자유다** —
	# 아니면 월드가 바뀌었을 때 몸이 영영 못 나온다.
	var p := WorldCollide.move(Vector2(127, 72), Vector2(-50, 0), _map(COAST))
	at(p, 77.0, 72.0, "겹친 채 벽에서 나오기")

func test_overlapping_start_is_not_pushed_back() -> void:
	# 겹친 채 벽 쪽으로 밀면 **되밀지 않는다**. 밀어내면 한 틱에 순간이동으로 보인다.
	var p := WorldCollide.move(Vector2(127, 72), Vector2(0.5, 0), _map(COAST))
	check(p.x >= 127.0, "겹친 채 벽 쪽 — 잰 값 %.4f · 기대 127 이상" % p.x)
...
func test_overlaps_sees_the_body_corners() -> void:
	...
	check(WorldCollide.overlaps(Vector2(168, 72), m), "막는 칸 한가운데는 겹친다")
	check(WorldCollide.overlaps(Vector2(140, 72), m), "모서리만 걸쳐도 겹친다")
	check(not WorldCollide.overlaps(Vector2(121.99, 72), m), "벽에 붙어 선 자리는 안 겹친다")
	check(not WorldCollide.overlaps(Vector2(168, 72), Callable()), "월드가 없으면 안 겹친다")
...
func test_tile_of_works_left_of_the_origin() -> void:
	...
	eq(WorldCollide.tile_of(47.99), 0, "47.99px")
	eq(WorldCollide.tile_of(48.0), 1, "48px")
