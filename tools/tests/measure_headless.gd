...
## CAMERA · CHOP · REGROW · DAY 를 차례로 잰다
## (회차 27 에 넷, 29 에 벌목, 30 에 다시 자라기, 31 에 낮과 밤).
##
...
## 돌았는지(7/7)**를 같이 적는다. `check.sh` 는 구간 줄 일곱과 이 줄을 **전부** 본다 —
## 하나라도 없으면 빨갛다.
##
...
const ORDER := ["move", "world", "collide", "camera", "chop", "regrow", "day"]
const PHASE_PATH := {
...
## 못 오므로 줄이 아예 안 나오고, 구간을 조용히 뺐으면 `구간 7/7` 이 `6/6` 으로 바뀐다.
## **세는 값을 `check.sh` 와 나눠 가진다**: 여기는 「몇 구간을 돌았나」만 찍고,
## 「일곱이어야 한다」는 `check.sh` 가 안다 — 한 파일만 고쳐서는 초록이 안 된다.
func _report() -> bool:
