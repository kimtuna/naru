...
## **실측 게이트 — 헤드리스 넷을 한 프로세스에서.** MOVE · WORLD · COLLIDE · CAMERA
## 를 차례로 잰다 (회차 27).
##
...
## 돌았는지(4/4)**를 같이 적는다. `check.sh` 는 구간 줄 넷과 이 줄을 **전부** 본다 —
## 하나라도 없으면 빨갛다.
##
...
const ORDER := ["move", "world", "collide", "camera"]
const PHASE_PATH := {
...
## 못 오므로 줄이 아예 안 나오고, 구간을 조용히 뺐으면 `구간 4/4` 가 `3/3` 으로 바뀐다.
## **세는 값을 `check.sh` 와 나눠 가진다**: 여기는 「몇 구간을 돌았나」만 찍고,
## 「넷이어야 한다」는 `check.sh` 가 안다 — 한 파일만 고쳐서는 초록이 안 된다.
func _report() -> bool:
