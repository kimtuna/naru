...
## **잰 폭 0.000** (9판 전부 0.0 · `jitter.sh 9`). 대조군 ⑤ 는 33px 을 옮기므로
## 여유 **8.3배**.
const POS_TOL := 4.0
...
const SKIP_BOX := 40.0
## 8비트로 두 칸. 렌더러가 반올림할 자리를 남긴다. **잰 폭 0.000** · 최대 0.003382
## (9판 · 서서·걷고·밤 셋 다). 막는 고장은 **해안의 물↔땅 색차 0.39** — 화면이 한 칸
## 미끄러지면 이만큼 튄다. 여유 **49.7배**.
const TOL := 2.0 / 255.0
...
# 한밤의 밝기 상한. **어둡기를 여기 다시 적지 않는다** — 얼마나 어두운가는 사람이 눈으로
# 정하는 값이고(회차 33: 0.29 → 0.18), 이 문이 묻는 것은 **밤이 낮과 구별되나** 하나다.
# 회차 33 에서 0.5 → 0.30 으로 좁혔다. 값을 따라 적은 게 아니라 **여유의 비를 되돌린
# 것이다**: 0.5 는 0.29 짜리 밤의 1.7 배였는데, 밤만 내리고 문을 그대로 두면 「밤을
# 세 배 가까이 밝혀도 초록」이 된다. 0.30 은 0.18 의 1.7 배라 그 폭이 예전과 같다.
# **0.18 로 되돌아가는 것까지는 못 잡는다** — 그건 이 문의 일이 아니다.
const NIGHT_MAX_LIGHT := 0.30
...
## 서로 다른 자리가 이만큼은 나와야 「모션」이다. 9판 최소 17 · 폭 1.
## 막는 고장은 자리 1개(붙박이) — 여유 **3배**. 표는 `test_tolerances.gd`.
const USE_MIN_SPOTS := 3
## 처음과 끝이 이만큼은 벌어져야 한다 (px). **하한이라 잰 값 아래에 있어야 한다** —
## 9판 두 벌: 최소 19.980 · **폭 1.595**(맨손 20.290~21.884 — 첫 벌은 0.536 만 봤다).
## 하한이라 잡음 아래 문턱은 19.980 − 1.595 = **18.385** 고 4.0 은 한참 아래다.
## 막는 고장은 붙박이 네모(≤ USE_SAME 0.5px) — 여유 **8배**.
const USE_MIN_SPAN := 4.0
