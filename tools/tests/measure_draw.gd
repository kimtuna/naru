...
##
## **다만 걷는 구간이 혼자 잡는 것은 아직 하나도 없다** (NUMBERS 9절). 아래의 순간이동이
## 이미 카메라를 옮겨 캐시를 버리게 만들어서, 대조군 4종이 전부 `[서서]` 에서 빨개진다.
## 지금 걷는 구간이 더하는 것은 **표본**이다: 물리가 민 진짜 재충전 3번을 지나고도
## 화면이 여전히 월드와 맞는가. 「걸어야만 깨지는 것」을 찾으면 그때 대조군이 된다.
##
