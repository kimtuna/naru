...
"""회차 기록을 다룬다.
  ...
  state.py tail <n>   최근 n 회차만 찍는다 (드라이버가 세션에게 주입한다)
  state.py roll <n>   최근 n 회차만 남기고 나머지는 .loop/archive/state.md 로 옮긴다

**세션은 state.md 를 열지 않는다.** 이 파일은 회차마다 자라므로, 통째로 읽게 두면
회차 비용이 계속 는다 — 1판이 그렇게 죽었다.
"""
import io, os, re, sys
...
def main():
    ...
    if cmd == "tail":
        ...
        if len(cycles) <= n:
            print("ROLL 유지 %d회차 (상한 %d)" % (len(cycles), n)); return
        ...
        print("ROLL %d회차를 아카이브로, %d회차 유지" % (len(old), len(keep)))
    else:
