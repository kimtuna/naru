...
# **남기는 절 수.** 세션에게 주는 것은 `tail 2`(아래 프롬프트 만드는 자리)라
# **3개를 남기면 세 번째 절은 아무도 안 읽으면서 약 30줄을 먹는다** — 기준 5 의 상한
# 90줄에서 여유를 그만큼 깎는다. 실측: 3개일 때 판정 시점 87~89줄(여유 1~3줄)이고,
# 세션이 절을 붙인 중간에는 ~116줄로 **가짜 빨강**이 됐다(GOTCHAS 에 적은 함정).
# 2개면 판정 ~60줄(여유 30줄) · 중간 ~89줄로 둘 다 초록이고 **세션이 보는 것은 그대로 2개**다.
# 기준 9 의 검사는 `tail ≤ keep` 을 묻는다 — 2 ≤ 2 로 통과한다.
STATE_KEEP=2
trim_state() {
  [ "$DRY" = "1" ] && return 0
...
full_redteam_if_due() {
  ...
  # ── **사람이 기계를 쓰고 있으면 미룬다** (2026-09-15 사람이 말했다) ─────────
  #
  # 쓸기는 대조군 116개가 각각 상태 검사를 돌리므로 **창이 100번 넘게 뜨고 50분**이
  # 걸린다. 창을 안 뜨게 하는 길은 없다 — 회차 13 이 여섯 가지를 재서 전부 뺏겼다
  # (NUMBERS 11절). 못 막으니 **때를 고른다**: 사람이 자리를 비운 뒤에 돈다.
  #
  # **미루기는 공짜다.** 여기서 그냥 돌아가면 다음 회차가 또 물어보고(아직 due 니까),
  # 그동안 루프는 **평소 일을 계속한다** — 멈추지 않는다. 시도 기록도 안 남긴다.
  #
  # **영영 미룰 수는 없다.** 사람이 며칠 붙어 있으면 죽은 게이트를 아무도 모른다.
  # `EVERY` 의 세 배가 지나면 **사람이 앞에 있어도 돈다** — 그때는 창이 뜨는 것보다
  # 게이트가 죽은 채 쌓이는 것이 비싸다.
  local force_at=$(( FULL_REDTEAM_EVERY * 3 ))
  [ "$force_at" -le 0 ] && force_at=24
  if [ $(( turn - last )) -lt "$force_at" ] \
     && ! bash tools/loop/idle.sh "$IDLE_FOR_SWEEP" >/dev/null 2>&1; then
    say "대조군 전체 쓸기를 미룬다 — $(bash tools/loop/idle.sh "$IDLE_FOR_SWEEP" 2>&1)"
    say "  ($why · 창이 100번 넘게 뜬다 · ${force_at}회차가 지나면 그래도 돈다)"
    return 0
  fi
