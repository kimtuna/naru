...
# **회차 36**: 비추는 점을 발밑으로 되돌린다. 화면은 멀쩡히 플레이어를 따라가고
# 편차도 0 이라 「카메라가 산다」는 판정은 전부 초록이다 — **12px 만 내려간다.**
# 몸통 한가운데를 묻지 않는 게이트는 여기서 아무 말도 못 한다.
mut scenes/player.tscn '^position = Vector2\(0, -12\)$' 'position = Vector2(0, 0)'
expect 1 "카메라를 발밑으로 되돌리면 잡는다 (씬의 글자 · 실측 둘 다)"
cp "$BAK/player.tscn" scenes/player.tscn
