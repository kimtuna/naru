...
IDLE_FOR_WINDOW="${IDLE_FOR_WINDOW:-60}"     # 창을 띄우기 전에 사람이 이만큼 쉬었어야 한다
                                         # (초). 평소 바퀴에도 창이 4번 뜬다 — 세션의 마지막
                                         # 확인 2번 + 판정 2번. 그걸 타이핑이 멈춘 틈에 넣는다
IDLE_FOR_WINDOW_MAX="${IDLE_FOR_WINDOW_MAX:-180}"  # 그래도 이만큼 지나면 띄운다 (초).
                                         # 사람이 내리 타이핑하면 세션이 45분 상한에 걸려
                                         # 통째로 버려진다 — 그게 창 몇 번보다 비싸다
