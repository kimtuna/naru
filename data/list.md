...
- [ ] 21. 밭 API 가 판을 아직 cell 이라 부른다 (G-145 단계 4)
  - 기준: 밭 API 가 판을 아직 cell 이라 부른다 — Farmland.is_tilled(cell) · till(cell) · aimed_ground()["cell"] · Crops 의 cell 인자가 이제 2m 밭 판 좌표인데 이름이 1m 칸 그대로라 위의 테스트처럼 헷갈려 잘못 넘긴다. 이름을 plate 로 바꾸거나(저장본 키는 이미 plate) 적어도 aimed_ground 의 키를 plate 로 바꿔라
