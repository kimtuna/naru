...
- 사람이 플레이해 보고 「건축은 전체적으로 다시 손봐야 한다」고 했다 (2026-09-24).
  ...
  러스트 · 발하임처럼 **소켓 + 걸음 수**로 바꾼다
- **먼저 걸음 수, 그다음 도면, 그다음 크기** 순서다 — 크기를 먼저 바꾸면 어긋난 것이 더 커진다
- [ ] 1. 장 수를 버리고 걸음 수로 잰다
  - 기준: `ROOF_SPAN` 과 「받치는 장 수」가 **코드와 spec 어디에도 없다** (테스트)
  - 기준: 땅에 닿은 토대가 **0걸음**, 거기 붙은 것이 1걸음, 그 옆이 2걸음이다 (테스트)
  - 기준: **제 등급의 걸음 수를 넘으면 놓이지 않는다** — 나무 3 · 석재 4 · 철 5 (임시) (테스트)
  - 기준: **기둥이 선 자리는 다시 0걸음**이 된다 — 거기서 다시 뻗을 수 있다 (테스트)
  - 기준: 받침을 부숴 걸음 수가 넘어가면 **무너진다** — 연쇄로 (테스트)
...
- [ ] 2. 저장까지 이어진다 — G-903 에서 옮겨 왔다
  ...
  - 기준: (앞 단계 QA) 나갔다 들어오면 주검이 새 사슴이 된다 — 섬의 짐승을 저장하지 않아(DONT_SAVE "Island/Animals", 까닭은 적혀 있다) 사슴을 잡고 섬을 나갔다 들어오면 집 자리에 성한 사슴이 바로 선다. 「동물도 자원처럼 다시 난다」인데 나무는 벤 상태를 담아 며칠 뒤에 다시 나는 것과 어긋나, 드나들기만 하면 하루를 안 기다리고 사냥감이 무한히 난다. 바닥에 재워 둔(가방에 넣기 전) 사슴도 사라진다. 막는 것은 아니다. game/life/hunting/island_animals.gd 에 집마다 「비어 있음 / 비운 날」 만이라도 저장본에 담아 다시 들어와도 빈 집은 다음 날까지 비어 있게 하고, test_island_animals.gd 에 「잡고 저장 → 새로 불러와도 그 집은 비어 있다, 하루 지나면 난다」 단언을 더하라. 그러면 DONT_SAVE 줄과 spec/04_life/hunting.md 22행 「저장하지 않는다」 · spec/01_settings/save.md 37행도 고쳐 적는다
...
- [ ] 3. 설치물도 놓을 자리가 보인다
  - 「모든 설치물들은 오버레이를 둘 것. 밭 · 연구대 · 상자 · 조리대 다」 — 지금 미리보기는 건축물뿐이다
...
- QA 가 통과시키면서 찾은 것이다. **물어볼 것이 아니면 여기로 온다** (사람 결정 2026-09-22).
  루프가 제 손으로 갚는다 — 사람은 순서를 바꾸고 싶을 때만 손댄다
- 기능을 더하지 않는다. 지키는 것이 없던 자리에 지키는 것을 넣는 일이다
- [ ] 낡은 Godot 함수를 걷어낸다 — `instance_reset_physics_interpolation()` 은 deprecated 다
...
- [ ] 3. 다쳐서 도망간 뒤 진정하지 않는다 (G-130 단계 3)
...
- [ ] 5. 총을 들면 단추의 「(우클릭)」이 틀린다 (G-131 단계 3)
...
- [ ] 6. 도축하지 않은 주검이 영원히 남는다 (G-131 단계 3)
...
- [ ] 7. 상자에 넣은 짐승은 영영 안 깬다 (G-132 단계 4)
...
- [ ] 10. chest 찍기에 제작대 창이 남아 겹친다 (G-129 단계 1)
...
- [ ] 17. 녹이는 곳 창 바닥이 넓게 비어 있다 (G-901 단계 1)
...
- [ ] 18. 창이 떠 있어도 위 안내 줄이 걷기 안내다 (G-901 단계 1)
...
- [ ] 19. 테스트가 GameRoot 의 밑줄 함수를 직접 부른다 (G-901 단계 1)
