...
- 저장본이 아직 시드 · 이름뿐이다 (`world_data.gd:6` — 「밭 · 목장 · 상자는 그것들을 만드는 단계에서 붙인다」).
  나갔다 들어오면 지은 것 · 넣어 둔 것이 전부 사라진다. spec/01_settings/save.md 는
  「월드: 내 섬 · 개척 섬들의 건축물 · 밭 · 목장 · 상자」라고 이미 적고 있다 (루프가 찾음 2026-09-22)
- [ ] 1. 놓은 것이 저장본에 남는다
  - 기준: 설치한 것(제작대 · 보관 상자 · 횃불)이 나갔다 들어와도 같은 자리에 같은 방향으로 선다 (테스트)
  - 기준: 간 밭과 심은 작물이 자란 만큼 그대로 남는다 (테스트)
  - 기준: 부순 것은 되살아나지 않는다 (테스트)
- [ ] 2. 상자 속과 데스 상자
  - 기준: 상자에 넣어 둔 것이 칸 그대로 남는다 (테스트)
  - 기준: 데스 상자는 남은 시간과 함께 저장되고, 불러온 뒤 이어서 줄어든다 (테스트)
...
- [ ] 50. 탄창이 총이 아니라 Attack 에 붙어 있다 (G-118 단계 1)
  - 기준: 탄창이 총이 아니라 Attack 에 붙어 있다 — Magazine 이 attack.gd 의 필드 하나(_magazine)라, 총을 내려놓았다 다시 들어도 · 가방의 다른 총으로 바꿔도 같은 탄창을 쓴다. 지금은 총이 한 자루뿐이라(spec 08 「총 종류 미정」) 드러나지 않지만, 총이 둘이 되면 한 자루에서 쏜 탄이 다른 자루에서도 빠져 있다. 이 경우를 보는 테스트도 없다. 총 종류가 정해질 때 탄창을 아이템 쪽으로 옮기고 「총 A 로 쏜 뒤 총 B 를 들면 B 는 가득 차 있다」를 짜야 한다
- [ ] 51. 섬의 붙이는 차례를 지키는 테스트가 없다 (G-118 단계 2)
...
- [ ] 54. 막대 크기와 소수 자릿수에 「임시」 표시가 없다 (G-121 단계 2)
...
- [ ] 55. 「닫아 둔 동안 시간이 흘렀다」 단언이 헐겁다 (G-121 단계 2)
...
- [ ] 56. 칸 하나를 눌러도 버퍼째로 가져오는 것이 spec 에 없다 (G-121 단계 2)
...
- [ ] 58. 갈고리총이 가득 찬 채 손에 들어오는 것이 임시라고 안 적혀 있다 (G-122 단계 2)
