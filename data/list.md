...
- spec: (없음 — 테스트를 빠르게 하는 일이다)
- 실측 2026-09-23: 전체가 **134개 파일 27분**(하루 전 87개 8분), 회차마다 도는 「바뀐 곳만」도 **7~12분**이다.
  섬을 한 번 세우는 데 5초쯤 걸리는데 그것을 **테스트마다** 다시 세우는 곳이 남아 있다
- 테스트가 **확인하는 것을 줄이지 않는다** — 세우는 횟수만 줄인다
- [x] 1. 무엇이 느린지 잰다
  - 기준: 테스트 **파일별로 걸린 시간**을 재서 느린 열 개를 report 에 적는다
  - 기준: 그중 **섬을 세우느라 느린 것**이 몇 개인지 가른다
  - 기준: 재는 방법을 다음 사람이 다시 쓸 수 있게 적어 둔다 (한 줄이면 된다)
- [ ] 2. 한 번만 세운다
  - 기준: 섬을 `before_each` 마다 세우던 것을 **`before_all` 에서 한 번**으로 바꾼다 —
    테스트끼리 섬 상태가 새지 않게 쓰고 나서 되돌린다 (테스트)
  - 기준: 바꾼 파일들의 **테스트 수와 단언 수가 그대로**다 — 확인하는 것을 줄이지 않았다 (테스트)
  - 기준: 전체 시간이 **눈에 띄게 준다** — 고치기 전후 시간을 report 에 적는다

## [ ] G-137 저장이 빠지지 않게 — 왕복 검사
