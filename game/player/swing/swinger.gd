...
## 손에 든 것이 휘두르지 않고 다른 동작을 하면(총 → 쏘기, 갈고리총 → 쏘기) 등록된 동작(add_alternate)이 맡는다.
...
## 좌클릭을 휘두르기 대신 맡는 동작들: func(item: Variant) -> float.
## 맡았으면 다음 동작까지 기다릴 초, 맡지 않으면 음수. 앞에 등록한 것부터 묻는다. 없으면 늘 휘두른다.
var _alternates: Array[Callable] = []
var _holding := false
var _cooldown := 0.0
var _show_left := 0.0
...
func add_alternate(action: Callable) -> void:
	_alternates.append(action)
...
func use() -> float:
	for action in _alternates:
		var wait: float = action.call(player.held_item())
		if wait >= 0.0:
			return wait
	swing()
