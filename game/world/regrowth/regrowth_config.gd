...
## spec/03_world/resources-regrowth.md: 주기는 나무 < 돌 < 광물(철 · 유황). 약초는 spec 에 없다 — 임시로 나무와 돌 사이.
## 날 수 · 둘레 반경은 spec 미정 — 임시 값이다.

const DEFAULT_PATH := "res://world/regrowth/regrowth_config.tres"
const Deposit := IslandConfig.Deposit

## 게임 하루의 초. 기본 설정은 load_default 가 DayNightConfig 값으로 맞춘다 — 하루 길이는 한 곳에만 둔다.
...
static func load_default() -> RegrowthConfig:
	var cfg := load(DEFAULT_PATH) as RegrowthConfig
	cfg.day_seconds = DayNightConfig.load_default().day_seconds
	return cfg
...
func period_of(deposit: Deposit) -> float:
	match deposit:
		...
		Deposit.IRON, Deposit.SULFUR:
			return ore_days * day_seconds
		Deposit.HERB:
			return herb_days * day_seconds
	return 0.0
