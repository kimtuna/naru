...
· [문서 전부 (`docs/`)](docs/) · [기획서](docs/GDD.md) · [바퀴 일지](docs/JOURNAL.md)

## 어떻게 만들고 있나 — 루프 엔지니어링

한 바퀴에 백로그 **한 줄**을 만든다. 사람이 아니라 **기계가 채점한다.**
...
- **새 검사를 만든 바퀴는 대조군을 돌린다** (`redteam.sh`). 일부러 깨뜨려서 빨개지는지 —
  안 하면 아무것도 안 잡는 가짜 게이트가 쌓인다
- 초록으로 끝난 항목의 verify 는 **계약에 영구 기준으로 승격**된다. 다음 바퀴부터 회귀도 잡힌다
- 사람만 답할 수 있는 `[ASK]` 항목은 **세션을 아예 열지 않는다**

```bash
tools/loop/ctl.sh start 5     # 5바퀴, 예산 안에서
...
**매 바퀴 읽는 것이 부풀면 바퀴가 갈수록 비싸진다.** 그래서 층을 갈랐다 — [docs/README.md](docs/README.md).
