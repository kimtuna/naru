...
##
## **보이는 칸만 그린다.** 월드는 256×256 = 65536 칸이고 한 화면에는 21 × 13 = 273 칸이
## 걸린다 — 240배다. 통째로 그려도 **사람 눈에는 똑같아 보인다**. 그래서 main.gd 가
## 몇 칸을 그렸는지 세어 두고 `measure_draw.gd` 가 그 수를 잰다. 픽셀만 봐서는 못 잡는다.
##
...
##   - 48px 격자가 눈에 보여야 240 px/s 가 5칸/초로 보이는지 **사람이 걸어서 확인**한다
