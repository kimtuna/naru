...
## 한밤의 빛. 밝기는 **0.18** 이고 b/r 은 **1.069** 다 (머리말의 여유 · 돌이 1.09).
## 더 파랗게 하고 싶으면 `test_day_cycle.gd` 가 먼저 빨개진다.
##
## **어둡기는 눈이 정한다** — 0.29 는 「밤 같긴 한데 저녁 같다」였다 (2026-09-15 사람).
## 채널 비는 그대로 두고 통째로 0.6207 배 했다: 부등식을 떠받치는 것은 밝기가 아니라
## **b/r** 이라, 같은 비로 줄이면 달빛 아래 바위는 그대로 바위다.
