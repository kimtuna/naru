...
##
## **차지한 칸은 비워 둔다** (회차 32): 다시 자라는 것을 미루는 물음 `occupied` 는
## 이제 「몸이 서 있나」가 아니라 **「사람이 차지했나」**다 — 목록은 `Claim` 이 든다.
##
...
## **사람이 차지하고 있어서** 못 자랐을 때 다시 물어보기까지의 시간.
## 매 프레임 다시 묻지 않는 이유는 비용이 아니라 **줄 서기**다: 못 자란 칸을 큐 맨 앞에
## 그대로 두면 그 뒤의 칸들이 영영 제 차례를 못 본다.
## 1초는 사람이 못 느끼는 길이다 — 비켜서면 그 자리에서 자란 것처럼 보인다.
## **안 움직이는 것(설치물)도 같은 줄에 선다**: 집을 허문 다음 날 그 자리에 나무가
## 돌아와야 하는데, 「영영 안 자라는 칸」으로 적어 두면 되돌릴 자리가 없어진다.
const RETRY_SEC := 1.0
...
## **「이 칸을 사람이 차지했나」**. 월드를 아는 쪽(main.gd)이 꽂아 준다.
## **비어 있으면 아무도 안 막는다** — 월드만 세우는 검사가 그대로 돈다.
##
## **몸만이 아니다** (회차 32): 설치물 · 간 밭 · 길 — 사람이 자리를 잡은 칸 전부다.
## 회차 30 이 이걸 넣을 때는 게임에 설치물이 하나도 없어서 물음이 「몸이 서 있나」
## 하나였는데, GDD A-4 가 요구하는 것은 그게 아니다. 여러 물음을 여기 if 로 쌓으면
## **월드가 설치물·밭·길을 전부 알게 된다** — 그래서 목록은 밖에 있다: `Claim` 이
## 이름 붙은 출처를 모아 하나의 Callable 로 답하고, 빠뜨린 것은 제가 센다.
...
func tick(delta: float) -> int:
	...
	while not _due.is_empty() and float(_due[0]["at"]) <= now:
		...
		if cleared.get(tile, INF) != e["at"]:
			continue
		# **사람이 차지한 칸에는 안 자란다** (GDD A-4). 몸이면 여기가 없을 때
		# 나무가 사람 안에서 자라서 「벤 자리에 몸이 낀다」가 되고 — 이 파일이
		# 막으려고 있는 바로 그것이다 — 설치물이면 **집 거실에 나무가 선다.**
		# 누가 차지했든 월드는 똑같이 미룬다: 목록은 `Claim` 의 것이다.
