...
## 이 파일을 안 열어도 된다.
##
## **없는 종류는 안 자란다.** 돌·광물이 캔 자리에 도로 생기면 광산과 자동화(A-4)가
## 통째로 의미를 잃는다 — 그쪽은 「깊이 파고 들어간다」로 푸는 자리다.
## 1.0 은 **자리표시자다** — `Harvest.DROP_AMOUNT` 의 3 과 같은 자리이고,
## 제작(P3)이 목재 수요에 값을 요구할 때 같이 정해진다.
const REGROW_DAYS := {
	TREE: 1.0,
}

## 이 종류가 다시 자라는 데 걸리는 날 수. **0 이면 안 자란다.**
