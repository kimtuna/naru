...
자동으로 읽어서, 옮기면 매 회차 운영 지침이 안 들어간다.

## 층 — 「누가 언제 읽나」가 파일마다 다르다

매 회차 읽는 것이 부풀면 회차가 갈수록 비싸진다. 그래서 문서를 네 층으로 갈랐다.

| 층 | 파일 | 누가 | 언제 |
|---|---|---|---|
| **고정** | [`/CLAUDE.md`](../CLAUDE.md) | Claude Code 자동 로드 | 매 회차 · 41줄 |
| **주입** | [`PROMPT.md`](PROMPT.md) | **드라이버가 넣는다.** 세션은 안 연다 | 매 회차 · 60줄 |
| **주입** | [`BACKLOG.md`](BACKLOG.md) | 드라이버가 **한 줄만** 뽑는다 | 매 회차 · 한 줄 |
...
| **사람** | [`JOURNAL.md`](JOURNAL.md) | **세션은 읽지 않는다** | 회차 끝에 절을 하나 덧붙인다 |
| 롤링 | `../.loop/state.md` | **드라이버** | 3회차 넘으면 아카이브 |

**얇음은 상태 검사가 강제한다.** `doclen.sh` 가 상태 검사 기준 5로 무장돼 있어서,
...
| **저번 회차에 뭐가 막혔고 그래서 뭘 바꿨나** | `JOURNAL.md` |
...
루프가 돌면 드라이버가 **초록으로 닫힌 회차**마다 굽고 커밋하고 **푸시한다.**
상태 검사 · 항목 verify · 일지를 다 통과한 뒤에만 나가므로 **빨간 상태는 바깥에 안 보인다.**
