...
- [ ] `measure_draw.gd` 의 **걷는 구간에 대조군을 붙이거나 그 구간을 뺀다** | verify: `tools/loop/redteam.sh`
- [ ] **`measure_facing.gd` 가 사람의 마우스에 안 흔들리게 한다** — 진짜 커서를 뺏어 재는
      탓에 사람이 만지면 `FACE 91.73°`(기대 90°)로 튄다. **바퀴 8 · 사람 세션 · 바퀴 9
      세 번 났다.** 커서를 안 쓰거나, 뺏은 동안 사람 입력을 막거나 | verify: `tools/loop/check.sh tests`
  > 게이트는 두 번 재는데 **대조군 4종이 전부 `[서서]` 에서 이미 빨개진다** — 해안까지의 순간이동이
  > 그 자체로 캐시를 버리게 만들기 때문이다. **걷는 구간이 혼자 잡는 것은 지금 하나도 없다**
  > (NUMBERS 9절). 「걸어야만 깨지는 것」을 한 줄로 주입할 방법을 못 찾았다.
