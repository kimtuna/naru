...
- [ ] 카메라가 플레이어를 따라간다. **줌 없음** (해상도가 시야 이득이 되면 안 된다) | verify: `tools/loop/check.sh tests`
  > 카메라가 오면 `main.gd` 의 `tile_offset`(화면 칸 → 월드 칸 = 118,123)이 **0 이 되어 사라진다.**
  > 플레이어를 스폰 칸(128,128)에 그대로 놓고, `test_player_scene.gd` 의 「시작 위치가 화면 안」
  > 줄도 그때 카메라 기준으로 바꾼다.
- [ ] **월드를 화면에 그린다** — 보이는 칸만. `main.gd` 의 임시 격자를 걷어낸다 | verify: `tools/loop/shot.sh /tmp/w.png`
  > **카메라가 먼저다 — 그래서 이 줄이 그 아래다.** 지금 시점은 원점을 비추는데 섬은 한가운데(128,128)에 있어서,
  > 지금 그리면 화면이 통째로 바다다. 카메라가 플레이어를 따라간 뒤에 그린다.
