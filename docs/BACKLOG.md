...
- [ ] **월드를 화면에 그린다** — 보이는 칸만. `main.gd` 의 임시 격자를 걷어낸다 | verify: `tools/loop/shot.sh /tmp/w.png`
  > **카메라가 왔으므로 이제 그릴 수 있다.** 그릴 범위는 `main.gd` 의 `visible_world_rect()` 가 준다
  > (지금 임시 격자가 그걸 쓴다 — 같은 자리에 진짜 타일이 얹힌다).
  > 스폰에서 가장 가까운 곧은 해안이 **101칸 밖**이라, 해안이 화면에 들어오려면 걸어가야 한다.
